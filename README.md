...
**Статус:** MVP бэкенда собран и проверен вживую — поток `событие/запрос → задача в БД →
карточка в YouGile` работает end-to-end (B-01…B-10, см. чек-лист внизу).
...
|-----------------------------------------------|---------------------------------------------------------|
| `GET /healthz`                                | проверка живости + число тенантов                       |
| `POST /v1/workspaces/{tenant}/credentials`    | онбординг: сохранить креды YouGile (ключ или логин/пароль) |
| `POST /v1/tasks`                              | создать одобренную задачу → карточка в YouGile          |
| `PATCH /v1/tasks/{id}`                        | сменить статус → движение/закрытие карточки (FR-7)      |
| `GET /v1/workspaces/{tenant}/tasks`           | список задач воркспейса (дайджест)                      |
| `POST /v1/events`                             | приём событий → в очередь (async, `202`)                |

Тело передаётся JSON-ом; ошибки — конверт `{"error":"..."}`. Сбой внешнего YouGile
после записи в БД отдаётся `502` вместе с сохранённой задачей (публикацию можно
повторить, не пересоздавая задачу).
...
- **Recovery-middleware**: паника в хендлере превращается в `500` со стек-трейсом в
  логе, сервер не падает.
- **Внешние ошибки YouGile** изолированы в клиенте (`APIError` со статусом и телом,
  таймаут 15с, лимит чтения тела) и маппятся на `409/502` с понятным сообщением.
- **Graceful shutdown**: по SIGINT/SIGTERM HTTP завершается аккуратно, затем очередь
  дренится (необработанных событий не теряем в пределах буфера).
...
- [x] **B-10** — структурное логирование запросов (статус/длительность), recovery-middleware, изоляция ошибок YouGile (`409`/`502`), graceful shutdown, финальный README
